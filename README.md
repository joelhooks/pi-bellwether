...
`start` accepts the same optional `prompt` field to launch a worker with its first task in one call. A single-line prompt is the final positional CLI argument after `agentArgs`; Bellwether passes it as one argv value, and Herdr shell-quotes argv at the terminal boundary. Herdr rejects control characters in startup arguments, so a multiline prompt starts the agent first and then submits the exact text through `agent.prompt`. With either path, Bellwether reports `proofOfLife` only after Herdr observes `working`, bounded by 30 seconds. A prompt failure reports that the agent already launched; do not blindly start it again.
