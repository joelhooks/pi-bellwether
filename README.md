...
`start` accepts the same optional `prompt` field to launch a worker with its first task in one call. Single-line prompts for Pi, Claude, and Codex (verified through each CLI's `--help`) are appended as one positional argv value after `agentArgs`; Herdr shell-quotes argv at the terminal boundary. Herdr rejects control characters in startup arguments, and positional prompt syntax is unverified for other harnesses. For multiline prompts or other harnesses, Bellwether starts the agent, waits for Herdr-confirmed interactive readiness, then submits the exact text through `agent.prompt`. With either path, Bellwether reports `proofOfLife` only after Herdr observes `working`, bounded by 30 seconds. A prompt failure reports that the agent already launched; do not blindly start it again.
